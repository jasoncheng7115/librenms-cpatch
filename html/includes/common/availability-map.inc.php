...
if (defined('SHOW_SETTINGS')) {
    ...
    <form class="form" onsubmit="widget_settings(this); return false;">
        <div class="form-group">
            <div class="col-sm-4">
                <label for="title" class="control-label availability-map-widget-header">小工具標題</label>
            </div>
            <div class="col-sm-6">
                <input type="text" class="form-control" name="title" placeholder="自訂小工具標題" value="'.htmlspecialchars($widget_settings['title']).'">
            </div>
        </div>';
    ...
    <div class="form-group">
        ...
        <div class="col-sm-6">
            <select class="form-control" name="color_only_select">
                <option value="1"' . ($widget_settings['color_only_select'] == 1 ? ' selected' : '')  . ' >是</option>
                <option value="0"' . ($widget_settings['color_only_select'] == 1 ? '' : ' selected')  . ' >否</option>
            </select>
        </div>
    </div>
';
    ...
    <div class="form-group">
        <div class="col-sm-4">
            <label for="show_disabled_and_ignored" class="control-label availability-map-widget-header">取消/關閉</label>
        </div>
        <div class="col-sm-6">
            <select class="form-control" name="show_disabled_and_ignored">
                <option value="1" '.$selected_yes.'>是</option>
                <option value="0" '.$selected_no.'>否</option>
            </select>
        </div>
    </div>';
    ...
    <div class ="form-group">
        <div class="col-sm-4">
            <label for="mode_select" class="control-lable availability-map-widget-header">模式</label>
        </div>
        <div class="col-sm-6">
            <select name="mode_select" class="form-control">';

    if ($config['show_services'] == 0) {
        $common_output[] = '<option value="0" selected>只有裝置</option>';
    } else {
    ...
    if ($directpage == "yes") {
        ...
        <div class="page-availability-title-left">
            <span class="page-availability-title">可用性地圖於</span>
            <select id="mode" class="page-availability-report-select" name="mode">';

        if ($config['show_services'] == 0) {
            $temp_header[] = '<option value="0" selected>只有裝置</option>';
        } else {
            foreach ($select_modes as $mode_select => $option) {
                if ($mode_select == $mode) {
                    $selected = 'selected';
    ...
    if ($show_disabled_ignored == 1) {
        $disabled_ignored_header = '
            <span class="label label-default label-font-border label-border">忽略: '.$host_ignored_count.'</span>
            <span class="label blackbg label-font-border label-border">關閉: '.$host_disabled_count.'</span>';
    }
    ...
    if ($mode == 0 || $mode == 2) {
        $temp_header[] = '
            <div class="' . $deviceClass . '">
                <span>裝置統計</span>
                <span class="label label-success label-font-border label-border">上線: '.$host_up_count.'</span>
                <span class="label label-warning label-font-border label-border">警告: '.$host_warn_count.'</span>
                <span class="label label-danger label-font-border label-border">離線: '.$host_down_count.'</span>
                '.$disabled_ignored_header.'
            </div>';
    }
    ...
    if (($mode == 1 || $mode == 2) && ($config['show_services'] != 0)) {
        $temp_header[] = '
            <div class="' . $serviceClass . '">
                <span>服務統計</span>
                <span class="label label-success label-font-border label-border">上線: '.$service_up_count.'</span>
                <span class="label label-warning label-font-border label-border">警告: '.$service_warn_count.'</span>
                <span class="label label-danger label-font-border label-border">離線: '.$service_down_count.'</span>
            </div>';
    }
